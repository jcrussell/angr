...
Status: **DONE — Option B, S1+S2+S3 landed** (human go/no-go 2026-06-25).
...
/ `handle_native_resume`) in `26f2bf6fc`. S3 (`angr-xxukz`, native `pthread_once`)
in `adea42e52`, plus the run-loop Path-A wiring (`96dfd2994`) that the S2 review
flagged — both native dispatch paths now route sub-call procs through `call_ex`.
E2e in `2a1df9d9f`. `pthread_create` `static_exits` remains out of scope (needs a
symbolic-branch spawn / CFG; pure symex has no CFG — risk #4 below).
...
- **S3 (xxukz) — DONE (`adea42e52` + `96dfd2994` + `2a1df9d9f`):** `NativePthreadOnce`
  (`procedures/pthread.rs`) implements `call_ex`/`resume`; reads the once-guard,
  sets the done-bit *before* the sub-call (recursion-safe), then
  `CallAndResume{target: func}`. `pthread_create` `static_exits` stays a follow-on
  (out of scope, risk #4).
  **S3 integration note (S2 review finding — now RESOLVED in `96dfd2994`):** S2
  wired `call_ex`/`CallAndResume` into `handle_simprocedure` (`stepping.rs`) only.
  The *other* native-proc dispatch path — the inline fast path at the top of the
  run loop (`run_loop.rs`, the `native_proc.call` arm ~L274) — called return-only
  `call`, so a sub-call proc reached via fresh entry (PC landing on the hook at
  loop top) would silently never sub-call. S3 routed that arm through `call_ex`
  and handles `CallAndResume` there too (capturing the caller return address via
  `get_return_addr` before `setup_native_subcall` overwrites the sentinel slot).
  The resume *return* (guest `ret` → sentinel) was always fine — the sentinel
  surfaces mid-block as `RunResult::SimProcedure`, landing in `handle_simprocedure`.
  Known limitation: Path A derives `caller_return_addr` from `[sp]` only (correct
  on amd64; link-register ABIs fall back, since `link_register()` is unwired).
