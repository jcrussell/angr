...
EXAMPLE_CATALOG = {
    ...
    "write_stream_heavy": {
        "tier": "fast",
        "rust_ok": True,
        "notes": "angr-csyy9: write-heavy cle-stream demonstrator. ~192 fputs/fputc/fwrite calls against the cle stdout/stderr FILE* externs on a single concrete path. Under Rust every write-side SimProcedure (stdio.rs::fputs, puts.rs::fputc, fwrite.rs::fwrite) hits an unmapped lazy page in read_fileno_for_stream -> ProcedureError::Memory -> Python fallback, so Rust is EXPECTED slower than Python here (known-regression demonstrator, not a speed win). Measured fallback cost is ~2.8ms/call (192 fallbacks, time_in_callbacks 0.534s; iter76 commit 5a75297ac) -- NOT the ~100ms/call the superseded write-side-fileno-fallback-correct memory estimated. Kept OUT of baseline_timings.json so the rust<python ratio does not trip the gate. Quantifies the write-side fileno fallback cost for csyy9 blocker (2). See bd memories write-stream-heavy-bench and write-side-fileno-fallback-correct.",
    },
