 ...
 * ProcedureError::Memory -> the dispatcher falls back to Python (~100ms per
 * call). The fallback is CORRECT (Python resolves the right fd) but slow, so
 * this loop deliberately amplifies the per-call cost into a measurable
 * regression. See bd memory write-side-fileno-fallback-correct.
 *
