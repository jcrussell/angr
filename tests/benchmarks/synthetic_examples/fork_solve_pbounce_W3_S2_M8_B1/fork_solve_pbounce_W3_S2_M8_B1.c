...
int main(void) {
    ...
    /* level 0: 1 mixing round(s) then a gated trap */
    acc = (acc * 1103515245u + 12345u) ^ (acc >> 3); acc += b[0];
    if (((s >> 0u) & 0x1u) == 0u) acc = trap_point(acc);
    /* level 1: 1 mixing round(s) then a gated trap */
    acc = (acc * 1103515245u + 12345u) ^ (acc >> 3); acc += b[1];
    if (((s >> 1u) & 0x1u) == 0u) acc = trap_point(acc);
    ...
    if ((acc & 0xffu) == 0xeeu) reach_target();
    return (int)acc;
}
