#!/usr/bin/env python
"""MIPS64 (big-endian) synthetic benchmark.
...
This is the big-endian counterpart of ``mips64_le_branch``. The
program shape is identical; the storage byte order differs
(``EI_DATA = MSB``, ``>I`` packing of instruction words, BE memory and
instruction endness in archinfo). Closes the last gap in the
``ig3o`` Arch BE validation epic by promoting MIPS64 BE from
Skeleton/Experimental to Supported.
...
immediate to 64 bits on MIPS64). The ELF wrapper is ELF64 with
``EI_CLASS=ELF64`` and ``EF_MIPS_ARCH_64`` exercising the N64 calling
convention path.
...
skipped; the regression gate still validates MIPS64 BE lift + exec
stays green and timing stays within 15% of baseline_timings.json's
cached ``rust_time``.
...
def _build_elf():
    """Return raw bytes of an ELF64 MIPS BE executable wrapping ``CODE``."""
    ehdr = (
        b"\x7fELF"
        + bytes(
            [
...
def solve():
    elf_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "mips64be_branch.elf")
    ...
    assert proj.arch.memory_endness == "Iend_BE", proj.arch.memory_endness
