...
def _child(out_q, example_name, n_iters):
    resource.setrlimit(resource.RLIMIT_AS, (4 * 1024 * 1024 * 1024, resource.getrlimit(resource.RLIMIT_AS)[1]))
...
def main():
    ...
    print("=== _overlay_relocated_sections wall-clock profile (fauxware) ===")
    print(f"Iterations: {result['n_iters']}")
    print(
        f"Per-call (ms): min={min(times):.3f} max={max(times):.3f} "
        f"mean={sum(times) / len(times):.3f} median={sorted(times)[len(times) // 2]:.3f}"
    )
    print()
    print("Section breakdown (one full sweep):")
    ...
    print()
    print("Total time breakdown (one full sweep):")
    ...
    print()
    print("Top 10 slowest sections by load time:")
