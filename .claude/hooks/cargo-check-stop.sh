...
# `cargo clippy --all-targets -- -D warnings` (default features — --all-features
# is intentionally omitted; the fuzzer feature is bit-rotted, see bd).
#
...
if ! out=$(cargo clippy --manifest-path native/angr/Cargo.toml --all-targets -- -D warnings 2>&1); then
  {
