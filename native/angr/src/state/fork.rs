...
/// (angr-9ke6b.121).
///
/// `pub(super)` because the `merge_field_*` bodies the
/// [`angr_macros::MergePolicy`] derive generates for `warn_on_diverge` fields
/// land in the parent module, next to the struct definition.
pub(super) fn warn_config_divergence(field: &str, diverged: bool) {
    if diverged {
        log::warn!(
...
impl RustSimState {
    ...
    pub fn merge(&self, others: &[&RustSimState], merge_conditions: &[RustBV]) -> Self {
        ...
        // `sim_options` and the four `#[merge_policy = "union"]` booleans in
        // the struct literal below are two views of the same Python option set,
        // so they merge the same way: union / logical-OR. An option a branch
        // switched on stays on in the merged state rather than being reverted
        // by whichever branch happened to be `self`.
        ...
        // means something mutated one branch's config mid-run. All but
        ...
        // Every `merge_field_*` below is generated by
        // `#[derive(angr_macros::MergePolicy)]` from that field's
        // `#[merge_policy = "..."]` label, so the label and the merge line
        // cannot disagree — a generated method `merge` forgets to call is
        // dead_code, which CI's `-D warnings` rejects. Fields whose policy the
        // derive can't express are written out here as before.
        RustSimState {
            arch: self.merge_field_arch(others),
            vex_arch: self.merge_field_vex_arch(others),
            registers: merged_regs,
            memory: merged_mem,
            solver: Rc::new(RefCell::new(merged_solver)),
            pc: self.merge_field_pc(others),
            state_id: next_state_id(),
            parent_id: Some(self.state_id),
            history: self.merge_field_history(others),
            detailed_history: self.merge_field_detailed_history(others),
            max_history: self.merge_field_max_history(others),
            hooks: merged_hooks,
            ...
            removed_hooks: Arc::new(HashSet::new()),
            concretizer: self.merge_field_concretizer(others),
            track_history: self.merge_field_track_history(others),
            fs: best_fs,
            ...
            // branch's ITE arm.
            heap_brk: self.merge_field_heap_brk(others),
            posix_brk: self.merge_field_posix_brk(others),
            mmap_base: self.merge_field_mmap_base(others),
            ...
            tsc_counter: self.merge_field_tsc_counter(others),
            getopt_optind: self.merge_field_getopt_optind(others),
            getopt_optchar: self.merge_field_getopt_optchar(others),
            getopt_extern: self.merge_field_getopt_extern(others),
            native_resume_stack: self.native_resume_stack.clone(),
            ...
            no_ip_concretization: self.merge_field_no_ip_concretization(others),
            no_symbolic_jump_resolution: self.merge_field_no_symbolic_jump_resolution(others),
            keep_ip_symbolic: self.merge_field_keep_ip_symbolic(others),
            force_eager_forks: self.merge_field_force_eager_forks(others),
            ...
            cgc_allocation_base: self.merge_field_cgc_allocation_base(others),
