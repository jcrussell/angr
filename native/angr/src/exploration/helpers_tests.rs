...
fn load_concrete_bytes_chunked_splits_at_16_and_concatenates() {
    ...
    let out = load_concrete_bytes_chunked(0x1000, 40, |a, n| {
        seen.push((a, n));
...
/// no later chunk is requested. All three call sites depend on this — a short
...
fn load_concrete_bytes_chunked_propagates_first_error() {
    ...
    let res = load_concrete_bytes_chunked(0, 48, |_a, n| {
        calls += 1;
        if calls == 2 {
            Err(PyValueError::new_err("unreadable"))
        } else {
...
fn load_concrete_bytes_chunked_zero_size_never_calls_source() {
    let mut calls = 0usize;
    let out = load_concrete_bytes_chunked(0x2000, 0, |_a, _n| {
        calls += 1;
        Ok(vec![0xff])
    })
