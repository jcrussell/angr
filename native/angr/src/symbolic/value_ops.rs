            ...
            match (self.as_u128(), amount.as_u128()) {
                (Some(v), Some(a)) => {
                    ...
                    if amt == 0 {
                        ...
                        // safe above the u128 storage ceiling too.
                        Self::concrete(v, w)
                    } else if w <= 128 {
                        Self::concrete((v $main amt) | (v $comp (w - amt)), w)
                    } else {
                        // A `Concrete` of `width > 128` stores only its low 128
                        // bits, so a non-trivial rotate would move set bits into
                        // positions the u128 payload cannot represent — and
                        // `v $main amt` with `amt >= 128` is an out-of-range
                        // shift on a u128 (debug abort, release wrap mod 128).
                        // Unlike shl/lshr/ashr, whose out-of-range results are
                        // all-zero and so *are* representable, there is no
                        // correct concrete answer here: decline the fold and
                        // keep the rotate symbolic, the same way `concat_into`
                        // keeps a wide concat as an expression.
                        Self::expr_node(w, $op, [self, amount])
                    }
                }
                _ => Self::expr_node(w, $op, [self, amount]),
            }
        }
    };
}
...
impl RustBV {
    ...
    // Rotates. The concrete arm reduces mod width before narrowing,
    // special-cases amt==0, and declines the fold above the u128 storage
    // ceiling (see `invariant-concrete-shift-clamp-before-narrow` and
    // `invariant-concrete-bv-u128-16-byte-limit`); rotl/rotr are exact mirror
    // images, generated by define_rotate_pair!.
    define_rotate_pair!(
...
pub(super) fn drive_extract<T: ExtractTarget>(
    ...
    // site, see bv_concrete::concrete_extract_u128).
