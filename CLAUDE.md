...
| `make check` | `cargo check --release` — fast type/borrow check. Default features only, so it does **not** cover `fuzzer`/`fuzzing`/`libvex-ffi` code; it is not the CI gate. Reproduce that with `cargo clippy --all-targets --all-features -- -D warnings` (run from the repo root with no `--manifest-path`, so `--all-targets` reaches `native/angr-macros`'s test targets too — angr-h3tx0). |
...
- **Compiler-enforced call-site invariants**: `native/angr-macros/` — proc-macro
  ...
  behavioural half is `memory/tests/merge_sidecars.rs`. The function-like
  macros `callback_setters!`, `inspect_test_entries!` and `inspect_dispatch!`
  cover `callbacks/` (angr-0jh0j.6, angr-6cp06.79): `callback_setters!`
  generates every `set_<slot>` PyO3 setter from the one `with_callback_fields!`
  list in `callbacks/mod.rs`; `inspect_test_entries!` derives each
