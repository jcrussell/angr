...
| `make check-no-z3` | `cargo check -D warnings` over the four `--no-default-features` combos CI's `rust_feature_flags` gates. Neither `make check` nor clippy `--all-features` compiles the `#[cfg(not(feature = "vex-engine-z3"))]` arms — this is the only local way to type-check them, and (since angr-sqfj8.139) the only gate holding them to the zero-warning bar. |
